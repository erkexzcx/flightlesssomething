...
func main() {
	...
	log.Printf("Starting server with GOGC=%d", gogc) //nolint:gosec // G706: gogc is an int from debug.SetGCPercent, not user-tainted
...
// Supports: KiB, MiB, GiB, TiB suffixes
...
func parseMemoryLimit(s string) int64 {
	...
	// Check for valid 3-character suffix (KiB, MiB, GiB, TiB)
	suffix := s[len(s)-3:]
	if suffix == "MiB" || suffix == "GiB" || suffix == "KiB" || suffix == "TiB" {
