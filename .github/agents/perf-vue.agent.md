...
2. **Main thread blocking**: CPU-intensive operations (stats calculations, large JSON parsing, data transformations) running on the main thread instead of in Web Workers, synchronous heavy loops in lifecycle hooks or event handlers
...
5. **Lazy loading and code splitting**: Views not using lazy imports via Vue Router, large components or libraries loaded eagerly when they could be deferred
...
2. Check for main-thread-blocking patterns — this app uses Web Workers for heavy computation; verify new heavy work follows the same pattern
...
5. Check that large data operations (benchmark data processing, stats calculation) are offloaded to workers
6. Verify lazy loading for new views/routes and heavy imports
