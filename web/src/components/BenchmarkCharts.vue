    ...
    <ul class="nav nav-tabs" id="chartTabs" role="tablist">
      ...
      <li class="nav-item" role="presentation">
        <button 
          ...
          type="button" 
          role="tab"
          @click="handleTabClick('advanced-filters')"
        >
      ...
      <div class="tab-pane fade" id="advanced-filters" role="tabpanel">
        <div class="container mt-4">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">
                <i class="fas fa-filter"></i> Advanced Filter Settings
              </h5>
              <p class="text-muted">
              ...
              <div class="filter-section mt-4">
                ...
                <div class="form-check mb-3">
                  ...
                  <label class="form-check-label" for="filterExtremeSpikesCb">
                    ...
                    <span v-if="settingsChanged" class="badge bg-warning text-dark ms-2">
                      <i class="fas fa-sync-alt"></i> Setting changed
                    </span>
                  </label>
                ...
                <div v-if="settingsChanged" class="alert alert-info mt-3" role="alert">
                  ...
                  Charts have been updated to reflect the new settings.
                </div>
              </div>
            </div>
          </div>
...
function calculatePercentileFPS(fpsData, percentile) {
  ...
  // Only apply filtering if the user has enabled it
  let processedFpsData = fpsData
  if (percentile >= 90 && localFilterExtremeSpikes.value) {
  ...
  nextTick(() => {
    switch (tabName) {
      ...
      case 'advanced-filters':
        // No rendering needed for advanced filters tab - it's just settings
        break
      default:
...
function handleFilterChange() {
  ...
  // Re-render all tabs that depend on this setting (FPS and Frametime tabs)
  // Only re-render tabs that have already been rendered
  nextTick(() => {
